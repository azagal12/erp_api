...
public sealed class LoginForm : Form
{
    ...
    public LoginForm()
    {
        ...
        card.Controls.Add(Ui.Stack(
            Ui.HeroTitle("TecnoStor ERP"),
            Ui.Subtitle("Clientes, ventas, caja e inventario en la nube"),
...
public sealed class MainForm : Form
{
    ...
    private readonly ComboBox _entryProduct = Ui.Combo(500);
    private readonly DateTimePicker _entryDate = new() { Width = 220, Format = DateTimePickerFormat.Short };
    private readonly NumericUpDown _entryCost = Ui.Money();
    private readonly NumericUpDown _entrySale = Ui.Money();
    private readonly TextBox _entryProvider = Ui.Text("", 500);
    private readonly TextBox _entrySerials = Ui.MultiText("", 500, 150);
    private readonly DataGridView _inventory = Ui.Grid();
    private readonly ComboBox _inventoryCategory = Ui.Combo(220, "GENERAL", "PROCESADOR", "PLACA", "RAM", "DISCO", "VIDEO", "FUENTE", "CASE", "MONITOR", "PERIFERICO");
    private readonly TextBox _inventorySearch = Ui.Text("", 260);
    private readonly ComboBox _inventoryProduct = Ui.Combo(360);
    private readonly TextBox _inventorySerial = Ui.Text("", 260);
    private readonly TextBox _inventoryProvider = Ui.Text("", 260);
    private readonly DateTimePicker _inventoryDate = new() { Width = 180, Format = DateTimePickerFormat.Short };
    private readonly ComboBox _inventoryState = Ui.Combo(180, "DISPONIBLE", "VENDIDO", "RESERVADO", "GARANTIA");
    ...
    private Control Header()
    {
        ...
        nav.Controls.Add(NavButton("inventario", "▦ Inventario"));
        ...
        var refresh = Ui.SecondaryButton("↻ Refrescar", 130);
    ...
    private Button NavButton(string key, string text)
    {
        ...
        {
            if (button.BackColor != Ui.Blue) button.ForeColor = Ui.Blue;
        };
        button.MouseLeave += (_, _) =>
        {
            var selected = _navButtons.Any(pair => pair.Value == button && pair.Key == _currentView);
            button.ForeColor = selected ? Color.White : Ui.Ink;
        };
    ...
    private Control ProductsView()
    {
        ...
        var delete = Ui.DangerButton("Eliminar producto seleccionado", 330);
        ...
        delete.Click += async (_, _) =>
        {
            if (_products.CurrentRow?.DataBoundItem is not Product product) return;
            if (MessageBox.Show($"Eliminar producto {product.Name}?", "Confirmar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes) return;
            await SafeRun(async () =>
            {
        ...
        var addEntry = Ui.PrimaryButton("Registrar entrada de series", 500);
        addEntry.Click += async (_, _) =>
        {
            if (_entryProduct.SelectedItem is not Product product)
            {
                MessageBox.Show("Selecciona un producto para agregar series.", "Inventario", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            await SafeRun(async () =>
            {
                await _api.AddProductEntryAsync(product.Id, new ProductEntryInput(_entryDate.Value, _entryCost.Value, _entrySale.Value, _entryProvider.Text, _entrySerials.Text));
                _entrySerials.Clear();
                _entryProvider.Clear();
                await RefreshProductsAsync();
                BindEntryProducts();
            });
        };
        _entryProduct.SelectedIndexChanged -= EntryProductChanged;
        _entryProduct.SelectedIndexChanged += EntryProductChanged;

        return Workspace(
            Ui.FormPanel("Productos", "Catalogo y entradas de inventario por fecha",
                Ui.SectionTitle("Crear producto"),
                Ui.Field("Producto", name),
                Ui.Two(Ui.Field("SKU / codigo", sku), Ui.Field("Stock inicial", stock)),
                Ui.Two(Ui.Field("Costo inicial", cost), Ui.Field("Precio venta inicial", sale)),
                Ui.Field("Series iniciales (una por linea)", serials),
                add,
                delete,
                Ui.Separator(),
                Ui.SectionTitle("Entrada de series"),
                Ui.Field("Producto existente", _entryProduct),
                Ui.Two(Ui.Field("Fecha ingreso", _entryDate), Ui.Field("Costo unitario", _entryCost)),
                Ui.Field("Precio venta sugerido", _entrySale),
                Ui.Field("Proveedor / nota", _entryProvider),
                Ui.Field("Series nuevas (una por linea)", _entrySerials),
                addEntry
            ),
    ...
    private Control InventoryView()
    {
        var saveSerial = Ui.PrimaryButton("Guardar serie", 150);
        var viewSerials = Ui.SecondaryButton("Ver series del producto", 210);

        _inventorySearch.TextChanged += (_, _) => BindInventory();
        _inventoryCategory.SelectedIndexChanged += (_, _) => BindInventory();
        _inventory.CellClick -= InventoryCellClick;
        _inventory.CellClick += InventoryCellClick;

        saveSerial.Click += async (_, _) =>
        {
            if (_inventoryProduct.SelectedItem is not Product product) return;
            await SafeRun(async () =>
            {
                await _api.AddProductEntryAsync(product.Id, new ProductEntryInput(_inventoryDate.Value, product.CostPrice, product.EffectiveSalePrice, _inventoryProvider.Text, _inventorySerial.Text));
                _inventorySerial.Clear();
                await LoadAllAsync();
                BindInventory();
            });
        };

        viewSerials.Click += (_, _) =>
        {
            if (_inventoryProduct.SelectedItem is Product product) ShowProductSerials(product);
        };

        BindInventoryProducts();
        BindInventory();

        var top = Ui.Card();
        top.Dock = DockStyle.Top;
        top.Height = 90;
        top.Controls.Add(Ui.Horizontal(
            Ui.Field("Categoria", _inventoryCategory),
            Ui.Field("Buscador pro", _inventorySearch),
            Ui.InfoText("Filtra por producto, codigo, categoria, marca, modelo o serie.")
        ));

        var entry = Ui.Card();
        entry.Dock = DockStyle.Top;
        entry.Height = 128;
        entry.Controls.Add(Ui.Stack(
            Ui.SectionTitle("Agregar serie / ingreso de producto"),
            Ui.Horizontal(
                Ui.Field("Producto", _inventoryProduct),
                Ui.Field("Serie", _inventorySerial),
                Ui.Field("Proveedor", _inventoryProvider),
                Ui.Field("Fecha ingreso", _inventoryDate),
                Ui.Field("Estado", _inventoryState),
                saveSerial,
                viewSerials
            )
        ));

        var content = Ui.TablePanel("Inventario simple", _inventory);
        var page = new Panel { Dock = DockStyle.Fill, BackColor = Ui.Page };
    ...
    private void BindProducts()
    {
        ...
        BindEntryProducts();
        BindInventoryProducts();
        if (_currentView == "inventario") BindInventory();
    }

    private void BindEntryProducts()
    {
        _entryProduct.DataSource = null;
        _entryProduct.DataSource = _productData.ToList();
        if (_entryProduct.SelectedItem is Product product)
        {
            _entryCost.Value = Math.Min(Math.Max(product.CostPrice, 0), _entryCost.Maximum);
            _entrySale.Value = Math.Min(Math.Max(product.EffectiveSalePrice, 0), _entrySale.Maximum);
        }
    }

    private void BindInventoryProducts()
    {
        _inventoryProduct.DataSource = null;
        _inventoryProduct.DataSource = _productData.ToList();
    }

    private void BindInventory()
    {
        var category = _inventoryCategory.Text;
        var search = _inventorySearch.Text.Trim();
        var rows = _productData
            .Where(product => category == "GENERAL" || product.Category.Equals(category, StringComparison.OrdinalIgnoreCase))
            .Where(product => string.IsNullOrWhiteSpace(search)
                ...
                || product.Model.Contains(search, StringComparison.OrdinalIgnoreCase)
                || product.Category.Contains(search, StringComparison.OrdinalIgnoreCase)
                || product.Serials.Any(serial => serial.Contains(search, StringComparison.OrdinalIgnoreCase)))
            .Select(product => new InventoryRow
            {
                ...
                StockSistema = product.Stock,
                SeriesDisponibles = product.Serials.Count
            })
            .ToList();

        _inventory.DataSource = null;
        _inventory.DataSource = rows;
        HideColumns(_inventory, "Id");
        Header(_inventory, ("Codigo", "Codigo"), ("Producto", "Producto"), ("Categoria", "Categoria"), ("Marca", "Marca"), ("Modelo", "Modelo"), ("StockSistema", "Stock"), ("SeriesDisponibles", "Series"));
        MakeReadOnly(_inventory);
    }

    private void InventoryCellClick(object? sender, DataGridViewCellEventArgs e)
    {
        if (e.RowIndex < 0 || _inventory.Rows[e.RowIndex].DataBoundItem is not InventoryRow row) return;
        var product = _productData.FirstOrDefault(item => item.Id == row.Id);
        if (product is not null) _inventoryProduct.SelectedItem = product;
    }

    private void ShowProductSerials(Product product)
    {
        var form = new Form
        {
        ...
        var grid = Ui.Grid();
        grid.DataSource = product.Serials.Select(serial => new SerialRow
        {
            Producto = product.Name,
            Serie = serial,
            Estado = "DISPONIBLE"
        ...
        MakeReadOnly(grid);
        form.Controls.Add(Ui.TablePanel("Series disponibles del producto", grid));
        form.ShowDialog(this);
    }

    private void EntryProductChanged(object? sender, EventArgs e)
    {
        if (_entryProduct.SelectedItem is not Product product) return;
        _entryCost.Value = Math.Min(Math.Max(product.CostPrice, 0), _entryCost.Maximum);
        _entrySale.Value = Math.Min(Math.Max(product.EffectiveSalePrice, 0), _entrySale.Maximum);
    }
...
public static class Ui
{
    ...
    {
        var button = Button(text, 360, Color.White, Blue);
    ...
    public static Panel Card()
    {
        ...
        panel.Paint += (_, e) =>
        {
            using var pen = new Pen(Color.FromArgb(230, 236, 245));
    ...
    public static Label HeroTitle(string text) => new()
    {
        Text = text,
    ...
    public static FlowLayoutPanel Horizontal(params Control[] controls)
    {
        var panel = new FlowLayoutPanel
        {
            Dock = DockStyle.Fill,
    ...
    public static Label InfoText(string text) => new()
    {
        Text = text,
        AutoSize = true,
        ForeColor = Muted,
        Font = new Font("Segoe UI", 9),
        Margin = new Padding(12, 28, 0, 0)
    };
    ...
    public static Control FormPanel(string title, string subtitle, params Control[] controls)
    {
        ...
        var allControls = new List<Control> { ModuleTitle(title), Subtitle(subtitle) };
    ...
    public static Label ModuleTitle(string text) => new()
    {
        Text = text,
...
public sealed class ApiClient
{
    ...
    public async Task AddProductEntryAsync(string productId, ProductEntryInput input) => await PostAsync($"api/products/{productId}/entries", input);
...
public sealed record ProductEntryInput(DateTime EntryDate, decimal CostPrice, decimal SalePrice, string Provider, string Serials);
...
public sealed class InventoryRow
{
    ...
    public int SeriesDisponibles { get; set; }
}

public sealed class SerialRow
{
    public string Producto { get; set; } = "";
