...
async function readPgDb() {
  ...
  const [clients, products, sales, items, payments] = await Promise.all([
    ...
    db.query("select id, name, sku, price, stock from products order by name asc"),
    ...
    db.query("select id, sale_id, product_id, name, price, qty from sale_items order by id asc"),
...
async function writePgDb(dbSnapshot) {
  ...
  try {
    ...
    for (const item of dbSnapshot.products) {
      await client.query(
        "insert into products (id, name, sku, price, stock) values ($1, $2, $3, $4, $5)",
        [item.id, item.name, item.sku, item.price, item.stock]
      );
    }
    for (const sale of dbSnapshot.sales) {
      ...
      for (const item of sale.items) {
        await client.query(
          "insert into sale_items (id, sale_id, product_id, name, price, qty) values ($1, $2, $3, $4, $5, $6)",
          [item.id, sale.id, item.productId, item.name, item.price, item.qty]
        );
      }
    }
  ...
  if (req.method === "POST" && pathname === "/api/sales") {
    ...
    const items = body.items.map((item) => {
      ...
      return {
        ...
        name: product.name,
        price: money(item.price ?? product.price),
