...
const initialData = {
  clients: [],
  products: [
    { id: "prod-1", name: "Producto ejemplo 1", sku: "P001", price: 25, costPrice: 18, salePrice: 25, stock: 20, serials: [] },
    { id: "prod-2", name: "Producto ejemplo 2", sku: "P002", price: 40, costPrice: 30, salePrice: 40, stock: 12, serials: [] }
  ],
  sales: [],
  payments: []
};
  ...
  if (process.env.DATABASE_URL) {
    ...
    if (products.rows[0].count === 0) {
      for (const product of initialData.products) {
        await db.query(
          "insert into products (id, name, sku, price, cost_price, sale_price, stock, serials) values ($1, $2, $3, $4, $5, $6, $7, $8)",
          [product.id, product.name, product.sku, product.price, product.costPrice || 0, product.salePrice || product.price, product.stock, JSON.stringify(product.serials || [])]
        );
      }
    }
    return;
...
async function readPgDb() {
  ...
  const [clients, products, sales, items, payments] = await Promise.all([
    ...
    db.query("select id, name, sku, price, cost_price, sale_price, stock, serials from products order by name asc"),
...
async function writePgDb(dbSnapshot) {
  ...
  try {
    ...
    for (const item of dbSnapshot.products) {
      await client.query(
        "insert into products (id, name, sku, price, cost_price, sale_price, stock, serials) values ($1, $2, $3, $4, $5, $6, $7, $8)",
        [item.id, item.name, item.sku, item.price, item.costPrice || 0, item.salePrice || item.price, item.stock, JSON.stringify(item.serials || [])]
      );
    }
